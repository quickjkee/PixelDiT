...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=8):
    if path.endswith('.npz'):
        ...
        files = [Image.open(f).convert('RGB') for f in file_paths]
...
def calculate_fid(
    ...
    m1, s1 = compute_statistics_of_path(path1, model, batch_size,
                                             dims, device, num_workers)
...
def calculate_fid_from_images(
    ...
    """Compute FID between in-memory generated images and precomputed stats.

    -- images      : np.ndarray [N, H, W, 3] uint8 (or list of HxWx3 uint8
                     arrays / PIL images); TF.ToTensor maps them to [0,1] CHW.
    -- stats_path  : path to a .npz with 'mu' and 'sigma' (the reference set).
    Returns the scalar FID (float).
    """
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    else:
